...
async function main(params) {
  ...
  try {
    ...
    let cursor = null;
    do {  
      let url = `${params.aemHost}/adobe/sites/cf/fragments`;
      if (cursor) {
        url += `?cursor=${cursor}`;
      }
      ...
      allFragments = allFragments.concat(data.items);
      cursor = data.cursor || null;
    } while (cursor);
    
    return {
      statusCode: 200,
      body: allFragments.filter(item => item.model?.id === params.modelId)
    };
    */
   const modelId = params.modelId;
   const query = `{"filter":{"modelIds":["${modelId}"]}}`;
   let all = [];
   let cursor = null;
    do {
    let url = `${params.aemHost}/adobe/sites/cf/fragments/search?query=${encodeURIComponent(query)}`;
    if (cursor) {
      url += `&cursor=${cursor}`;
    }
